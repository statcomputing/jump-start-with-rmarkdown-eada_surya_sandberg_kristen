...
true_value <-numeric(length(t_values))
  ...
  estimate100[i] <- signif(monte_carlo_normal_cdf(100,t_values[i]), digits = 3)
  estimate1000[i] <- signif(monte_carlo_normal_cdf(1000,t_values[i]), digits = 3)
  estimate10000[i] <- signif(monte_carlo_normal_cdf(10000,t_values[i]), digits = 3)
  true_value[i] <- signif(pnorm(t_values[i]), digits = 3)
  }
