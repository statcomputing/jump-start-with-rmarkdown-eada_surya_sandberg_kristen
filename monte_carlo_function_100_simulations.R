monte_carlo_100_simulations<-function () 
{
  ...
  bias     <- numeric(length(n_values)*length(t_values)*100)
  ...
  for (j in 1:length(n_values)){
    ...
    for (i in 1:length(t_values)){
      for (m in 1:100){   
        ...
        bias[k] <- estimate[k] - pnorm(t_column[k])
        k = k+1
      }
    }
  ...
  output_matrix <-cbind(n_column,t_column,estimate,bias)
  #plot(t100~t_values,main="Car Milage Data", 
